...
@Client.on_message(filters.private & ~filters.command(["start", "help", "login", "logout", "forward", "forwardstop", "stats", "ping"]), group=2)
async def forward_state_handler(client: Client, message: Message):
    ...
    if not state or state not in ["awaiting_first_link", "awaiting_last_link", "awaiting_target", "awaiting_verification_forward"]:
